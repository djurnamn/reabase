...
local function capture_fx_parameters(track)
  ...
  for i = 0, fx_count - 1 do
    ...
    for p = 0, num_params - 1 do
      ...
      params[tostring(p)] = { name = name or ("param_" .. p), value = val }
    end
...
--- Uses reaper.defer() to allow a frame for plugins to initialize after SetTrackStateChunk.
---@param track MediaTrack
...
local function apply_fx_parameters(track, parameter_maps, callback)
  -- Defer to allow a frame for FX to initialize after SetTrackStateChunk
  reaper.defer(function()
    ...
    log("  apply_fx_parameters: applied params to " .. applied .. " FX")
...
local function apply_chunk_and_parameters(track, chunk, parameter_maps, preset, reabase_path)
  -- Apply structural chunk (creates FX with default state)
...
local function assign_preset(preset_name, replace)
  ...
  local snap_chunk, snap_err = bridge.snapshot(reaper_chunk, preset_name, state.reabase_path, capture_fx_parameters(state.track))
...
local function revert_plugin(slot_id)
  ...
  -- Apply the parameter map via TrackFX_SetParam
  reaper.Undo_BeginBlock()
  if result and result.parameterMap and result.pluginIndex then
    local fx_index = result.pluginIndex -- already 0-based
    ...
    if state.selected_preset_index == 0 then
      ...
      if reaper.ImGui_Button(ctx, "Assign") then
        ...
        if inspect and inspect.currentChain and #inspect.currentChain > 0 then
          ...
          -- Empty track: apply preset chain immediately (replace=true)
...
local function render_fx_table()
  ...
  -- Determine columns
  local col_count = 3  -- Move, Plugin, Type
  ...
  if reaper.ImGui_BeginTable(ctx, "fxchain", col_count,
    ...
    for i, fx in ipairs(chain) do
      ...
      if i > 1 then
        if reaper.ImGui_Button(ctx, "Up##up_" .. i) then
          -- Swap FX positions on the actual REAPER track
      ...
      if i < fx_count then
        ...
        if reaper.ImGui_Button(ctx, "Dn##dn_" .. i) then
      ...
      reaper.ImGui_TableNextColumn(ctx)
      -- Plugin name: clickable to open FX UI on preset tabs
    ...
    if has_merge then
      for _, action in ipairs(inspect.merge.actions) do
        if action.type == "add_base" and action.fx then
          reaper.ImGui_TableNextRow(ctx)
          reaper.ImGui_TableNextColumn(ctx)  -- Move (empty for merge rows)
          if show_ownership then
            reaper.ImGui_TableNextColumn(ctx)
          end
          if show_create_checkboxes then
