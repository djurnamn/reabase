...
-- Path to the reabase CLI binary. Resolved dynamically:
-- 1. REABASE_CLI_PATH environment variable (if set)
-- 2. `which reabase` lookup
-- 3. Common nvm/node locations on macOS
-- 4. Fallback to bare "reabase" (assumes it's in PATH)
bridge.cli_path = (function()
  -- Check environment variable
  local env_path = os.getenv("REABASE_CLI_PATH")
  if env_path and env_path ~= "" then return env_path end

  -- Try `which` (works if shell profile sets up PATH)
  local handle = io.popen("which reabase 2>/dev/null")
  if handle then
    local result = handle:read("*a"):match("^%s*(.-)%s*$")
    handle:close()
    if result and result ~= "" and not result:match("not found") then return result end
  end

  -- Try common nvm locations
  local home = os.getenv("HOME") or ""
  local nvm_dir = os.getenv("NVM_DIR") or (home .. "/.nvm")
  local nvm_handle = io.popen('ls -d "' .. nvm_dir .. '/versions/node/"*/bin/reabase 2>/dev/null | tail -1')
  if nvm_handle then
    local result = nvm_handle:read("*a"):match("^%s*(.-)%s*$")
    nvm_handle:close()
...
--- Inspect a track by sending its chunk and FX parameters to the CLI.
...
---@return string|nil error
function bridge.inspect_track(track_chunk, reabase_path, fx_parameters)
  local path_arg = reabase_path and (' -p "' .. reabase_path .. '"') or ""
  local input = json.encode({
    trackChunk = track_chunk,
    fxParameters = fx_parameters,
  })
  local output, code = run_cli("inspect" .. path_arg, input)
  if not output or output == "" then
...
---@return table|nil result {modifiedChunk, parameterMaps}
---@return string|nil error
...
function bridge.save_preset(track_chunk, preset_name, reabase_path, selected_indices, extends_preset, overwrite, add_after, fx_parameters)
  local path_arg = reabase_path and (' -p "' .. reabase_path .. '"') or ""
  local payload = {
    trackChunk = track_chunk,
...
---@return string|nil error
function bridge.update_presets(track_chunk, ownership, released, reabase_path, fx_parameters)
  local path_arg = reabase_path and (' -p "' .. reabase_path .. '"') or ""
  local input = json.encode({
    trackChunk = track_chunk,
    ownership = ownership,
...
--- Returns the parameter map and plugin index for Lua to apply via TrackFX_SetParam.
---@param track_chunk string The full track chunk
---@param slot_id string The slot ID of the plugin to revert
---@param reabase_path string|nil Optional path to search from
---@return table|nil result {parameterMap, pluginIndex}
---@return string|nil error
