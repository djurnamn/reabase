...
function parseContentLine(trimmed: string): RppStruct | RppRawLine {
  ...
  if (match) {
    // Guard against base64 blob lines being misclassified as structs.
    // Base64 lines are long single tokens (100+ chars) with no spaces.
    // RPP struct tokens are short (the longest known is ~30 chars like
    // "nvk_take_source_type_v2") and always have params or are short standalone.
    // A single-word line longer than 64 chars is almost certainly base64 data.
