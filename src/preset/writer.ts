...
import { serializeRfxChainFromFingerprints } from "./rfxchain.js";
import { hashBlob } from "../snapshot/capture.js";

/**
 * Regenerate a root preset's rfxchain and plugins list from current fingerprints.
 */
export function updateRootPreset(
  ...
  const rfxChainPath = resolve(presetsDirectory, definition.fxChainFile);

  // Regenerate rfxchain from fingerprints
  const rfxContent = serializeRfxChainFromFingerprints(ownedFingerprints);
  writeFileSync(rfxChainPath, rfxContent, "utf-8");
 ...
 * - If slotId exists in parent chain and state differs: write state file + override entry
 * - If slotId exists in parent chain and state matches: remove override (inherits naturally)
 * - If slotId not in parent chain: add entry + include in child rfxchain
 */
export function updateChildPreset(
  ...
  for (const fp of ownedFingerprints) {
    if (parentSlotIds.has(fp.slotId)) {
      ...
      if (parentFp && parentFp.stateHash !== fp.stateHash) {
        // State differs — write state file and add override
        const stateFileName = `fx/${definition.name}_${fp.slotId}.state`;
        const stateFilePath = resolve(presetsDirectory, stateFileName);
        writeFileSync(stateFilePath, fp.stateBlob, "utf-8");
  ...
  if (additions.length > 0) {
    // Write rfxchain for added plugins
    const rfxChainRelPath = `fx/${safeFilename}.rfxchain`;
    const rfxChainAbsPath = resolve(presetsDirectory, rfxChainRelPath);
    const rfxContent = serializeRfxChainFromFingerprints(additions);
    writeFileSync(rfxChainAbsPath, rfxContent, "utf-8");

    yamlDefinition.fxChainFile = rfxChainRelPath;
