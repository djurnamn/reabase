...
export interface PresetDefinition {
  ...
  extends?: string;
  /** Relative path to the .rfxchain file (from the presets/ directory).
   *  Required when no `extends` is set. Optional when extending a parent. */
  fxChainFile?: string;
  /** Slot ID list — position maps to FX in rfxchain */
  plugins?: { id: string }[];
  /** Per-slot state override (keyed by slot ID) */
  ...
  /** @deprecated Per-FX overrides (keyed by plugin identity: "TYPE::NAME"). Use `override` with slot IDs instead. */
