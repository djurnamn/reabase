...
import { inspectTrack, applyChunk, setPreset, snapshotTrack, savePreset, deletePreset, revertPlugin, updatePresets } from "./commands/bridge.js";
...
import type { ApplyChunkInput, SetPresetInput, SnapshotInput, SavePresetInput, DeletePresetInput, RevertPluginInput, UpdatePresetsInput } from "./commands/bridge.js";
...
program
  ...
  .action(async (options: { path: string }) => {
    ...
    try {
      const trackChunk = await readStdin();
      const result = inspectTrack(trackChunk.trim(), reabasePath);
      console.log(JSON.stringify(result));
    } catch (error) {
      console.error(
        JSON.stringify({ error: (error as Error).message })
