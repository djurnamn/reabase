...
export function inspectTrack(
  ...
  // If no slot map, resolve slotIds against the preset's chain.
  // This adopts the preset's slotIds for matching plugins via identity.
  if (!slotMapJson) {
  ...
  // Filter to only preset-managed slotIds (present in the resolved chain).
...
export function snapshotTrack(
  ...
  // Resolve slotIds: prefer stored slot map, fall back to preset's chain.
  // The fallback is critical on first snapshot when no slot map exists yet —
  // it adopts the preset's slotIds for matching plugins via identity (type + name).
  const slotMapJson = getExtState(track, "reabase_slot_map");
  if (slotMapJson) {
